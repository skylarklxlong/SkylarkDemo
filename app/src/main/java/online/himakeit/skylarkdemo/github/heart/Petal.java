...
/**
 * Created by：LiXueLong 李雪龙 on 2017/11/13 14:10
 * <p>
 * Mail : skylarklxlong@outlook.com
 * <p>
 * Description:花瓣类Petal
 */
public class Petal {
    ...
    public void render(PointInfo p, int radius, Canvas canvas) {
    ...
    //绘制花瓣，参数p是花芯的圆心的坐标
    private void draw(PointInfo p, Canvas canvas) {
        if (!isFinished) {

            path = new Path();
            //将向量（0，radius）旋转起始角度，第一个控制点根据这个旋转后的向量计算
            PointInfo t = new PointInfo(0, this.radius).rotate(MyUtil.degrad(this.startAngle));
            //第一个端点，为了保证圆心不会随着radius增大而变大这里固定为3
            PointInfo v1 = new PointInfo(0, 3).rotate(MyUtil.degrad(this.startAngle));
            //第二个端点
            PointInfo v2 = t.clone().rotate(MyUtil.degrad(this.angle));
            //延长线，分别确定两个控制点
            PointInfo v3 = t.clone().mult(this.stretchA);
            PointInfo v4 = v2.clone().mult(this.stretchB);
